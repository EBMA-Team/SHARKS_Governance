...
::: {fig-traffic}
![SHARKSRegistry.com traffic Nov-Apr](Website%20Traffic%20Apr%202025.png)
:::
...
Table1a <- gtsummary::tbl_cross( # For inline text retrieval in epoxy)
  CurrSTROBE_AF1,
  row = PISCFStatus1,
  col = AdminStatus2
)
...
NewMissed <- as.numeric(gtsummary::inline_text(Table1a, row_level = "Patient added to database", col_level = "Not Actioned"))
NewTotal <- as.numeric(gtsummary::inline_text(Table1a, row_level = "Patient added to database", col_level = "stat_0"))
...
FigureIKDC <- PROMTFI |>
  ...
  stat_halfeye() + 
   # Add a red ellipse to highlight scores above 42 at Preop
  ggforce::geom_ellipse(aes(x0 = which(levels(PROMTFI$TimePoint) == "Preop"), 
                    y0 = 80, 
                    a = 0.3, 
                    b = 5, 
                    angle = 0),
  ...
  # Add an annotation label
  annotate("text", 
           x = which(levels(PROMTFI$TimePoint) == "Preop") + 0.4, 
           y = 90,
           label = "High preop scores",
           color = "red",
           fontface = "bold",
...
FigureOHS <- PROMHA |>
  ...
  stat_halfeye() + 
   # Add a red ellipse to highlight scores above 42 at Preop
  ggforce::geom_ellipse(aes(x0 = which(levels(PROMHA$TimePoint) == "Preop"), 
                    y0 = 45, 
                    a = 0.3, 
                    b = 5, 
                    angle = 0),
  ...
  # Add an annotation label
  annotate("text", 
           x = which(levels(PROMHA$TimePoint) == "Preop") + 0.4, 
           y = 48,
           label = "High preop scores",
           color = "red",
           fontface = "bold",
...
The poor followup can be attributed to the slow rollout of automatic followup of the registry system for records created prior to the introduction of SHARKSv2 - these have been set to *inactive* to ensure thorough testing of the system. Records meeting these criteria have only been set to *active* as they have returned to clinic since the rollout. In the last week, all of these records have been sent PROMs links and reminders to enable scheduled followup.
