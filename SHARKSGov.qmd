...
A static registry snapshot was retrieved and formatted based on the fixed date of preparation of the snapshot (31-Dec-2025).
...
specific_snapshot <- get_specific_snapshot("20260917")
...
::: {#fig-traffic}
![Figure 1: SHARKSRegistry.com traffic Jul 2025 - Dec 2025](Website%20Traffic%20Mar%202026.png)
:::
...
CurrentDate <- as.character("20260917")
...
RecordCount <- dplyr::summarise(
  ...
  PatientCreationNum = as.numeric(PatientCreationDate)
    ...
    !(PatientCreationDate < ymd("20170701"))
  )
...
```{r}
#| label: ka-intraop-prep



SnapshotKASurg <- Snapshot |> dplyr::filter(
    ...
    ProcedureGroup = case_when(
      # Primary Bilateral
      stringr::str_detect(str_to_lower(ProcedureName), "kap2")                          ~ "Total Primary Bilateral",
      stringr::str_detect(str_to_lower(ProcedureName), "kap2.2")                          ~ "Total Primary Subsequent Bilateral",
      str_detect(str_to_lower(ProcedureName), "bilateral")                     ~ "Total Primary Bilateral",

      # Primary Unilateral
      str_detect(ProcedureName, "KAP1")                          ~ "Total Primary Unilateral",

      # Patellofemoral
      str_detect(ProcedureName, "PATELLOFEMORAL")                ~ "Unicompartmental Patellofemoral",
      
      # Unicompartmental
      str_detect(str_to_lower(ProcedureName), "unicompartmental") & str_detect(str_to_lower(ProcedureName), "kar1|revision", negate = TRUE)         ~ "Unicompartmental",

      # Best-guess primary — ambiguous loose names
      ProcedureName %in% c(
        "TKR",
        "TOTAL KNEE ARTHROPLASTY",
        "TOTAL KNEE ARTHROPLASY",        # typo variant in your data
        "KNEE ARTHROPLASTY",
        "COMPLEX PRIMARY TOTAL KNEE",
        "COMPLEX PRIMARY TOTAL KNEE REPLACEMENT - VALGUS",
        "COMPLEX TOTAL KNEE REPLACEMENT",
        "COMPLEX TOTAL KNEE REPLACEMENT + REMOVAL OF METAL",
        "ATTUNE TOTAL KNEE REPLACEMENT",
        "1ST STAGE REVISION TOTAL KNEE REPLACEMENT",
        "SECOND STAGE REVISION TOTAL KNEE REPLACEMENT"
      )                                                           ~ "Total Primary Unilateral",

      # Staged revisions — best guess revision
      ProcedureName %in% c(
        "1ST REVISION TOTAL KNEE REPLACEMENT",
        "REVISION KNEE REPLACEMENT",
        "REVISION OF TOTAL KNEE ARTHROPLASTY"
      ) | str_detect(str_to_lower(ProcedureName), "kar1") & str_detect(str_to_lower(ProcedureName), "unicompartmental", negate = TRUE)                                                           ~ "Total Revision Unilateral",
